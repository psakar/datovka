...
class MvRtpoVtData {
	Q_DECLARE_TR_FUNCTIONS(MvRtpoVtData)
...
QList<Gov::FormField> MvRtpoVtData::allFields(void) const
{
...
Gov::SrvcMvRtpoVt::SrvcMvRtpoVt(void)
    : Service()
{
	m_formFields = MvRtpoVtData().allFields();
}
...
bool Gov::SrvcMvRtpoVt::setFieldVal(const QString &key, const QString &val)
{
...
QByteArray Gov::SrvcMvRtpoVt::binaryXmlContent(void) const
{
	...
	xml.replace("__repl_SUBJECT_IC__", fieldVal(IC_KEY));
	return xml.toUtf8();
}
