...
{
	QJsonObject jsonRoot(jsonHierarchyNode(QString(), QString(),
	    QStringList()));

	{
		QStringList metadata; metadata << "jednička" << "1";
		QJsonObject jsonSub(jsonHierarchyNode(
		    QStringLiteral("První klient"), QString(), metadata));

		{
			QStringList metadata; metadata << "jednička" << "v šuplíku";
			QJsonObject jsonSubSub(jsonHierarchyNode(
			    QStringLiteral("První spis"),
			    QStringLiteral("af51ef52yx"), metadata));

			if (!jsonHierarchyAppendSub(jsonSub, jsonSubSub)) {
				return QByteArray();
			}
		}

		{
			QStringList metadata; metadata << "jednička" << "taky v šuplíku";
			QJsonObject jsonSubSub(jsonHierarchyNode(
			    QStringLiteral("Druhý spis"),
			    QStringLiteral("af51ef52yz"), metadata));
		...
		if (!jsonHierarchyAppendSub(jsonRoot, jsonSub)) {
			return QByteArray();
		}
	}

	{
		QStringList metadata; metadata << "dvojka" << "2";
		QJsonObject jsonSub(jsonHierarchyNode(
		    QStringLiteral("Druhý klient"), QString(), metadata));

		QJsonObject jsonSubSub(jsonHierarchyNode(
		    QStringLiteral("Další spis"), QStringLiteral("aabbccddee"),
		    QStringList()));
	...
	return QJsonDocument(jsonRoot).toJson(QJsonDocument::Indented);
}
