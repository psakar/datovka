...
QString Isds::createOwnerName(const Isds::DbOwnerInfo &dbOwnerInfo)
{
	QString name;
	switch (dbOwnerInfo.dbType()) {
	case Isds::Type::BT_FO:
		if (!dbOwnerInfo.personName().isNull()) {
			name = QString(dbOwnerInfo.personName().firstName()) +
			    " " + QString(dbOwnerInfo.personName().lastName());
		}
		break;
	...
	case Isds::Type::BT_PFO_AUDITOR:
		if (!dbOwnerInfo.personName().isNull()) {
			QString firmName(dbOwnerInfo.firmName());
			if (firmName.isEmpty() || (firmName == " ")) {
				...
				name = QString(dbOwnerInfo.firmName());
			}
		}
		break;
	default:
...
QString Isds::createOwnerAddressWithoutIc(const Isds::Address &address)
{
	QString adrs;
	if (!address.isNull()) {
		QString street(address.street());
		QString adNumberInStreet(address.numberInStreet());
		QString adNumberInMunicipality(
		    address.numberInMunicipality());

		if (street.isEmpty() || (street == " ")) {
			adrs = QString(address.city());
		} else {
			adrs = street;
			if (adNumberInStreet.isEmpty() ||
			    (adNumberInStreet == " ")) {
				adrs += + " " + adNumberInMunicipality
