...
#include "src/models/new_messages_model.h"

const int NewDbMsgsTblModel::m_rowAllocationIncrement(128);

NewDbMsgsTblModel::NewDbMsgsTblModel(enum NewDbMsgsTblModel::Type type,
    QObject *parent)
...
int NewDbMsgsTblModel::rowCount(const QModelIndex &parent) const
{
	if (parent.isValid()) {
		return 0;
	} else {
...
int NewDbMsgsTblModel::columnCount(const QModelIndex &parent) const
{
	if (parent.isValid()) {
		return 0;
	} else {
...
QVariant NewDbMsgsTblModel::data(const QModelIndex &index, int role) const
{
	int dataType;
...
bool NewDbMsgsTblModel::setHeaderData(int section, Qt::Orientation orientation,
...
QVariant NewDbMsgsTblModel::headerData(int section, Qt::Orientation orientation,
    int role) const
{
	int dataType;
...
bool NewDbMsgsTblModel::setType(enum NewDbMsgsTblModel::Type type)
{
	m_type = type;
...
void NewDbMsgsTblModel::setQuery(QSqlQuery &query)
{
	beginResetModel();
...
const QVector<QString> &NewDbMsgsTblModel::rcvdItemIds(void)
{
...
const QVector<QString> &NewDbMsgsTblModel::sntItemIds(void)
{
	static QVector<QString> ids;
	if (ids.size() == 0) {
	    ids.append("dmID");
...
bool NewDbMsgsTblModel::setRcvdHeader(void)
{
...
bool NewDbMsgsTblModel::setSntHeader(void)
{
...
QVariant NewDbMsgsTblModel::_headerData(int section,
