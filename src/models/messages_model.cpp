 ...
 * from a table.
 */
const QVector<QString> DbMsgsTblModel::receivedItemIds = {
    "dmID", "dmAnnotation", "dmSender", "dmDeliveryTime", "dmAcceptanceTime",
    "read_locally", "is_downloaded", "process_status"};


const QVector<QString> DbMsgsTblModel::sentItemIds = {
    "dmID", "dmAnnotation", "dmRecipient", "dmDeliveryTime", "dmAcceptanceTime",
    "dmMessageStatus", "is_downloaded"};
...
int DbMsgsTblModel::columnCount(const QModelIndex &parent) const
{
	switch (m_type) {
	case DUMMY_RECEIVED:
		return receivedItemIds.size();
		break;
	case DUMMY_SENT:
		return sentItemIds.size();
		break;
	default:
...
bool DbMsgsTblModel::setRcvdHeader(void)
{
	for (int i = 0; i < receivedItemIds.size(); ++i) {
		...
		if (msgsTbl.attrProps.find(receivedItemIds[i]) !=
			...
			/* Description. */
			this->setHeaderData(i, Qt::Horizontal,
			    smsgdtTbl.attrProps.value(receivedItemIds[i]).desc);
			/* Data type. */
			this->setHeaderData(i, Qt::Horizontal,
